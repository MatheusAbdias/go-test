...
func Test_app_authenticate(t *testing.T) {
	var theTests = []struct {
		...
		{
			"invalid user",
			`{"email":"admin@someotherdomain.com","password":"secret"}`,
	...
	for _, e := range theTests {
		...
		if e.expectedStatusCode != rr.Code {
			t.Errorf(
				"%s: returned wrong status code; expected %d but got %d",
				e.name,
...
func Test_app_userHandlers(t *testing.T) {
	...
	for _, e := range tests {
		...
		if rr.Code != e.expectedStatus {
			t.Errorf(
				"%s: wrong status returned; expected %d but got %d",
				e.name,
				e.expectedStatus,
				rr.Code,
			)
...
func Test_app_refreshUsingCookie(t *testing.T) {
	...
	for _, e := range tests {
		...
		if rr.Code != e.expectedStatus {
			t.Errorf(
				"%s: wrong status code returned; expected %d but got %d",
				e.name,
				e.expectedStatus,
				rr.Code,
			)
	...
	for _, c := range rr.Result().Cookies() {
		if c.Name == "__Host-refresh_token" {
			...
			if c.Expires.After(time.Now()) {
				t.Errorf("cookie expirations in future, and should not be: %v", c.Expires.UTC())
			}
		}
	}
